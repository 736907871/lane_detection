...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        for j in range(len(tmp_line)):
            lines.append(list(map(float,tmp_line[j])))  # [[x1, y1, x2, y2, ...], [], [], []] 长度不一定， 但于于 56*2
        
        ks = np.array([calc_k(line) for line in lines])             # get the direction of each lane 注意：求斜率时，坐标是图像素素，注意正负。
        ...
        if len(k_neg) == 1:                                           # for only one lane in the left
            ...
            draw(label,lines[which_lane],2)   # 左边 最右
            bin_label[1] = 1
        elif len(k_neg) == 2:                                         # for two lanes in the left
            which_lane = np.where(ks == k_neg[1])[0][0]
            draw(label,lines[which_lane],1)  # 左边第1条
            which_lane = np.where(ks == k_neg[0])[0][0]
            draw(label,lines[which_lane],2)  # 左边第2条
            bin_label[0] = 1
            bin_label[1] = 1
        elif len(k_neg) > 2:  #取最靠右的2条                                             # for more than two lanes in the left,
            which_lane = np.where(ks == k_neg[1])[0][0]                # we only choose the two lanes that are closest to the center
            draw(label,lines[which_lane],1)  # 斜率绝对值第二大， 左边第1条
            which_lane = np.where(ks == k_neg[0])[0][0]
            draw(label,lines[which_lane],2)  # 斜率绝对值最大， 左边第2条
            bin_label[0] = 1
            bin_label[1] = 1

        if len(k_pos) == 1:                                            # For the lanes in the right, the same logical is adopted.
            which_lane = np.where(ks == k_pos[0])[0][0]
            draw(label,lines[which_lane],3)  # 右边 最左
            bin_label[2] = 1
        elif len(k_pos) == 2:
            which_lane = np.where(ks == k_pos[1])[0][0]
            draw(label,lines[which_lane],3)  # 右边第1条
            which_lane = np.where(ks == k_pos[0])[0][0]
            draw(label,lines[which_lane],4)  # 右边第2条
            bin_label[2] = 1
            bin_label[3] = 1
        elif len(k_pos) > 2:
            which_lane = np.where(ks == k_pos[-1])[0][0]
            draw(label,lines[which_lane],3)  # 斜率最大， 右边第1条
            which_lane = np.where(ks == k_pos[-2])[0][0]
            draw(label,lines[which_lane],4)  # 斜率第二大， 右边第2条
            bin_label[2] = 1
            bin_label[3] = 1
